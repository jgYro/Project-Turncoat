...
async (page) => {
  ...
  await c.route('**/api/**',async route=>{
    ...
    if(url.endsWith('/api/drilldown/scan')||url.endsWith('/api/drilldown/review')){
      const kind=url.endsWith('/review')?'ai':'keywords';if(kind==='ai'){modelCalls++;if(failReview)return reply({error:{message:'Synthetic model unavailable.'}},503);}
      ...
      report.rawResponse=JSON.stringify(report.result);reports.push(report);return reply(report);
    }
  ...
  try{
    ...
    await p.getByRole('button',{name:'Scan keywords',exact:true}).click();await p.getByText('Carbon fiber · 1',{exact:true}).waitFor();assert(modelCalls===0,'Keyword scanning never invokes AI');
    ...
    assert(requests.find(x=>x.url.endsWith('/api/drilldown/scan')).body.includePdf===true,'Keyword scan includes PDF context by default');
    await p.locator('.drill-panel:not([hidden]) select[id$="-preset"]').selectOption('missile-supply');await p.getByRole('button',{name:'Run AI review',exact:true}).click();await p.getByText('A dual-use material',{exact:true}).waitFor();
    assert(modelCalls===1&&reports.at(-1).preset==='missile-supply','Selected AI preset sent exactly once');assert(await p.getByRole('heading',{name:'Direct references',exact:true}).isVisible()&&await p.getByRole('heading',{name:'Potential indirect relevance',exact:true}).isVisible(),'AI review separates direct and indirect findings');
    ...
    await p.getByRole('button',{name:'Run AI review',exact:true}).click();await p.locator('.drill-panel:not([hidden])').getByText('A dual-use material',{exact:true}).waitFor();assert(reports.at(-1).node==='paper','Paper reviews use the selected paper');
    ...
    const count=await p.locator('#workspace-tabs [role=tab]').count();await p.reload();await p.getByRole('button',{name:'Run AI review',exact:true}).waitFor();assert(await p.locator('#workspace-tabs [role=tab]').count()===count,'Workspace tabs survive reload');assert(modelCalls===2,'Reload never repeats AI');
    ...
    failReview=true;await p.getByRole('button',{name:'Run AI review',exact:true}).click();await p.getByText('Synthetic model unavailable.',{exact:true}).waitFor();assert(await p.getByRole('button',{name:'Run AI review',exact:true}).isEnabled(),'Model errors allow retry');
    ...
    await p.getByRole('tab',{name:'Provider searches',exact:true}).click();await p.getByRole('cell').filter({hasText:'杨超 <img src=x>'}).waitFor();assert(await p.locator('.research-table img').count()===0,'Saved queries are rendered as text');assert(modelCalls===3&&expansions===1,'Settings runs no provider search or AI');
